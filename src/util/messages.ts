...
export class Messages {
    ...
    static async showSupportGCodeMessage() {
        ...
        if (result != null) {
            ...
            if (result === actions[0]) {
                await env.openExternal(Uri.parse('https://marketplace.visualstudio.com/items?itemName=appliedengdesign.vscode-gcode-syntax&ssr=false#review-details'));
    ...
    static async showWhatsNewMessage(version: string) {
        ...
        if (result != null) {
            ...
            if (result === actions[0]) {
                ...
                await env.openExternal(Uri.parse('https://github.com/appliedengdesign/vscode-gcode-syntax/blob/master/CHANGELOG.md'));
            } else if (result === actions[2]) {
                await env.openExternal(Uri.parse('https://github.com/sponsors/appliedengdesign'));
            }
        }
    ...
    private static async showMessage(type: 'info' | 'warn' | 'error', msg: string, ...actions: MessageItem[]): Promise<MessageItem | undefined> {
